...
function activarComboCompleto(comboId) {
    ...
    filterPosIni = combo.posIni || null;
    filterPosFin = combo.posFin || null;
...
function compararCombos(a, b) {
    return (a.figura || '').localeCompare(b.figura || '')
...
function actualizarEtiquetasFiltros(candidatos) {
    ...
    const mostrarPosIni = filterPosIni;
    posIniEl.innerHTML = mostrarPosIni
        ? `<span class="pos-dot pos-dot-ini"></span>${mostrarPosIni}`
        : `<span class="pos-dot pos-dot-ini"></span>Cualquier Posición Inicial [🌈]`;

    const posFinEl = document.getElementById('posfin-selected');
    const mostrarPosFin = filterPosFin;
    posFinEl.innerHTML = mostrarPosFin
        ? `<span class="pos-dot pos-dot-fin"></span>${mostrarPosFin}`
...
function renderPosIniList() {
    const listEl = document.getElementById('posini-list');
    const candidatos = combosFiltrados('posIni');
    const valores = [...new Set(candidatos.map(c => c.posIni).filter(Boolean))]
        .sort((a, b) => a.localeCompare(b));

    let html = `<div class="dropdown-item ${filterPosIni === null ? 'selected' : ''}" data-value=""><span class="pos-dot pos-dot-ini"></span>Cualquier Posición Inicial [🌈]</div>`;
    if (valores.length > 0) {
        html += valores.map(p => `
            <div class="dropdown-item ${p === filterPosIni ? 'selected' : ''}" data-value="${p}"><span class="pos-dot pos-dot-ini"></span>${p}</div>
        `).join('');
    }
    listEl.innerHTML = html;
    listEl.querySelectorAll('.dropdown-item[data-value]').forEach(item => {
        item.onclick = () => {
            filterPosIni = item.dataset.value === '' ? null : item.dataset.value;
...
function renderPosFinList() {
    const listEl = document.getElementById('posfin-list');
    const candidatos = combosFiltrados('posFin');
    const valores = [...new Set(candidatos.map(c => c.posFin).filter(Boolean))]
        .sort((a, b) => a.localeCompare(b));

    let html = `<div class="dropdown-item ${filterPosFin === null ? 'selected' : ''}" data-value=""><span class="pos-dot pos-dot-fin"></span>Cualquier Posición Final [🌈]</div>`;
    if (valores.length > 0) {
        html += valores.map(p => `
            <div class="dropdown-item ${p === filterPosFin ? 'selected' : ''}" data-value="${p}"><span class="pos-dot pos-dot-fin"></span>${p}</div>
        `).join('');
    }
    listEl.innerHTML = html;
    listEl.querySelectorAll('.dropdown-item[data-value]').forEach(item => {
        item.onclick = () => {
            filterPosFin = item.dataset.value === '' ? null : item.dataset.value;
...
document.getElementById('posini-selected').onclick = (e) => {
    e.stopPropagation();
    toggleDropdown('posini-options-panel', () => renderPosIniList());
};
document.getElementById('posfin-selected').onclick = (e) => {
    e.stopPropagation();
    toggleDropdown('posfin-options-panel', () => renderPosFinList());
};
...
function valoresDisponiblesDimension(dimension) {
    ...
    const crudos = candidatos.map(c => c[campo]).filter(Boolean);
...
function cambiarPorFlechas(direccion) {
    ...
    else if (dimension === 'posIni') renderPosIniList();
    else if (dimension === 'dificultad') renderDificultadList();
    else renderPosFinList();
